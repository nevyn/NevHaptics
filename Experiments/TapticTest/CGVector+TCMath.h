...
static inline CGVector TCVectorFromPoint(CGPoint p) {
	return CGVectorMake(p.x, p.y);
}

static inline CGVector TCVectorFromSize(CGSize s) {
	return CGVectorMake(s.width, s.height);
}
